...
export async function createLibraryItem(item: Omit<LibraryItem, "id" | "created_at" | "updated_at">) {
  ...
  return data[0] as LibraryItem
}

export async function updateLibraryItem(id: string, updates: Partial<Omit<LibraryItem, "id" | "created_at">>) {
  const { data, error } = await supabase
    .from("library_items")
