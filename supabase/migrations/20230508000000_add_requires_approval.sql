-- Add requires_approval field to tasks in action_flows
-- Since tasks are stored in JSONB, we need to create a function to update them

-- Create a function to add requires_approval field to all tasks
...
BEGIN
  ...
  LOOP
    ...
    -- Update each section's tasks to include requires_approval
    ...
    LOOP
      ...
      IF jsonb_typeof(flow_record.sections->i->'tasks') = 'array' THEN
        ...
        LOOP
          -- Add requires_approval if it doesn't exist
